...
pcpp::MacAddress Net_core::arp(pcpp::IPv4Address ip) {
    ...
    return pcpp::NetworkUtils::getInstance().getMacAddress(ip, dev_, arpResponseTimeMS,
                                                           dev_->getMacAddress(), dev_->getIPv4Address(),
                                                           arpTimeOut);
}
...
void Net_core::start_arp_poisoning(pcpp::IPv4Address iface_ip, pcpp::IPv4Address vic_src_ip,
    ...
    if (vic_src_ip == iface_ip) {
        ...
        vic_src_mac = arp(vic_src_ip);
        if (vic_src_mac == pcpp::MacAddress::Zero) {
            logger_.log("didnt receive arp reply from " + vic_src_ip.toString() + "\n");
            return;
        }
    }
    ...
    if (vic_dst_ip == iface_ip) {
        ...
        vic_dst_mac = arp(vic_dst_ip);
        if (vic_dst_mac == pcpp::MacAddress::Zero) {
            logger_.log("didnt receive arp reply from " + vic_dst_ip.toString() + "\n");
            return;
        }
    }
    ...
    if (forward_to_ip == iface_ip) {
        ...
        if (forward_to == pcpp::MacAddress::Zero) {
            logger_.log("didnt receive arp reply from " + forward_to_ip.toString() + "\n");
            return;
        }
    }

    add_live_thread([&](const std::shared_ptr<bool> &alive) {
...
void Net_core::add_live_thread(std::function<void(std::shared_ptr<bool>)> func) {
    ...
    live_threads[last_added_thread_id] = std::make_shared<std::thread>(func,
                                                                       live_thread_alive[last_added_thread_id]);
...
void Net_core::start_arp_poison_detection(pcpp::IPv4Address iface_ip) {
    ...
    add_live_thread([=,this](const std::shared_ptr<bool> &alive) {
