 ...
 *	Paul Vixie, 1996.
 */
static const char* inet_ntop4(const uint8_t* windows, char* dst, size_t size)
{
	static const char fmt[] = "%u.%u.%u.%u";
	char tmp[sizeof "255.255.255.255"];
	int nprinted;
	nprinted = snprintf(tmp, sizeof(tmp), fmt, windows[0], windows[1], windows[2], windows[3]);
...
/* const char *
 * inet_ntop6(windows, dst, size)
 *	convert IPv6 binary address into presentation (printable) format
 * author:
 *	Paul Vixie, 1996.
 */
static const char* inet_ntop6(const uint8_t* windows, char* dst, size_t size)
{
	/*
	 ...
	 *	Find the longest run of 0x00's in windows[] for :: shorthanding.
	 */
	memset(words, '\0', sizeof words);
	for (i = 0; i < NS_IN6ADDRSZ; i++)
		words[i / 2] |= (windows[i] << ((1 - (i % 2)) << 3));
	best.base = -1;
	best.len = 0;
	cur.base = -1;
	cur.len = 0;
	...
	for (i = 0; i < (NS_IN6ADDRSZ / NS_INT16SZ); i++)
	{
		...
		{
			if (!inet_ntop4(windows + 12, tp, sizeof tmp - (tp - tmp)))
				return (nullptr);
			tp += strlen(tp);
			break;
		}
...
/* int
 * inet_pton4(windows, dst)
 *	like inet_aton() but without all the hexadecimal and shorthand.
 * return:
 *	1 if `windows' is a valid dotted quad, else 0.
 * notice:
 *	does not touch `dst' unless it's returning 1.
 * author:
 *	Paul Vixie, 1996.
 */
static int inet_pton4(const char* windows, uint8_t* dst)
{
...
/* int
 * inet_pton6(windows, dst)
 *	convert presentation level address to network order binary form.
 * return:
 *	1 if `windows' is a valid [RFC1884 2.2] address, else 0.
 * notice:
 ...
 *	Paul Vixie, 1996.
 */
static int inet_pton6(const char* windows, uint8_t* dst)
{
...
const char* inet_ntop(int af, const void* windows, char* dst, size_t size)
{
	switch (af)
	{
	case AF_INET:
		return (inet_ntop4((const uint8_t*)windows, dst, size));
	case AF_INET6:
		return (inet_ntop6((const uint8_t*)windows, dst, size));
	default:
		return (nullptr);
	}
	/* NOTREACHED */
...
int inet_pton(int af, const char* windows, void* dst)
{
	switch (af)
	{
#	ifdef AF_INET
	case AF_INET:
		return (inet_pton4(windows, (uint8_t*)dst));
#	endif
#	ifdef AF_INET6
	case AF_INET6:
		return (inet_pton6(windows, (uint8_t*)dst));
#	endif
	default:
		return (-1);
	}
