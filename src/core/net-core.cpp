...
ashk::Net_core::Net_core()=default;


int ashk::Net_core::discover_interface() {
...
std::vector<pcpp::PcapLiveDevice *> ashk::Net_core::discover_interfaces() {
return pcpp::PcapLiveDeviceList::getInstance().getPcapLiveDevicesList();
}
pcpp::IPv4Address ashk::Net_core::interface_ip() {
discover_interface();
return dev_->getIPv4Address();
}

void ashk::Net_core::start_arp_poisoning(pcpp::IPv4Address iface_ip, pcpp::IPv4Address vic_src_ip,
...
void ashk::Net_core::add_logger_method(const std::function<void(const std::string &)> &method) {
logger_.add_log_method(method);
}


void ashk::Net_core::end_task(int id) {
tasks[id]->end();
utils::Logger::getInstance().log("Sent end signal to task\n");

}

std::vector<int> ashk::Net_core::get_running_tasks() {
std::vector<int> out;
...
void ashk::Net_core::start_arp_poison_detection(pcpp::IPv4Address iface_ip) {
...
void ashk::Net_core::send_arp_req(pcpp::IPv4Address iface_ip, pcpp::IPv4Address ip) {
...
pcpp::MacAddress ashk::Net_core::arp(pcpp::IPv4Address ip) {
return NetUtils::arp(ip,dev_);
}


void ashk::Net_core::start_mitm_forwarding(pcpp::IPv4Address iface_ip
...
void ashk::Net_core::start_vlan_hopping(pcpp::IPv4Address iface_ip, int outer_id, int inner_id) {
    tasks[last_added_task_id]=std::make_unique<tasks::VlanHoppingTask>(dev_,iface_ip,outer_id,inner_id,last_added_task_id);
    tasks[last_added_task_id]->start();
    last_added_task_id++;
}

void ashk::Net_core::start_dtp_negotiation(pcpp::IPv4Address iface_ip, const std::string &domain_name) {
    tasks[last_added_task_id]=std::make_unique<tasks::DTPNegotiation>(dev_,iface_ip,domain_name,last_added_task_id);
    tasks[last_added_task_id]->start();
    last_added_task_id++;
}

void ashk::Net_core::start_dtp_domain_extraction(pcpp::IPv4Address iface_ip, char *buffer) {
    tasks[last_added_task_id]=std::make_unique<tasks::DtpDomainExtraction>(dev_,iface_ip,buffer,last_added_task_id);
    tasks[last_added_task_id]->start();
    last_added_task_id++;
}

void ashk::Net_core::start_detecting_wifi_aps(std::string iface_name_or_ip,std::vector<WifiAp> &ap_list) {
    tasks[last_added_task_id]=std::make_unique<tasks::WifiApScanningTask>(dev_,iface_name_or_ip,ap_list,last_added_task_id);
    tasks[last_added_task_id]->start();
    last_added_task_id++;
}

std::string ashk::Net_core::interface_name() {
    discover_interface();
    return dev_->getName();
}

void ashk::Net_core::start_detecting_wifi_hosts(const std::string &iface_ip_name_str,
...
void ashk::Net_core::start_sending_deauth_packets(const std::string &iface_ip_name_str,
                    WifiAp* wifi_ap,
                    std::vector<std::shared_ptr<WifiHost>> &host_list) {
...
void ashk::Net_core::start_password_cracking(const std::string &iface_ip_name_str, std::shared_ptr<HandShakeData> handshake_data) {
...
void ashk::Net_core::start_wpa2_handshake_capturing(const std::string &iface_ip_name_str, std::shared_ptr<HandShakeData> handshake_data) {
