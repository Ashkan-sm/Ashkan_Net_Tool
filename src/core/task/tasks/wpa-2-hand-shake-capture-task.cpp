...
ashk::tasks::WPA2HandShakeCaptureTask::WPA2HandShakeCaptureTask(pcpp::PcapLiveDevice *dev,std::string iface_name_or_ip,
                                                                std::shared_ptr<HandShakeData> data, int last_task_id): Task(last_task_id),
                                                                dev_(dev),
...
void ashk::tasks::WPA2HandShakeCaptureTask::exec() {
    ...
    if (!capture_wrapper.start_capture(dev_, PacketReceiver::onPacketArrivesWPA2HandShakeCapturing
                                       , &cookie
                                       ,last_task_id)) {
        return;
    }
